...
func BenchmarkExperimental(b *testing.B) {
	// 准备测试上下文
	ctx := context.WithValue(context.Background(), env.BusinessKey, "test_business")
...
func TestExperimental(t *testing.T) {
	...
	ctx = context.WithValue(ctx, env.BusinessKey, "helper_test")
	ctx = context.WithValue(ctx, env.PlatformKey, env.DP_MacOS)
	ctx = context.WithValue(ctx, env.DeviceKey, env.Dev_PC)
	ctx = context.WithValue(ctx, env.VersionKey, "v0.0.1")
